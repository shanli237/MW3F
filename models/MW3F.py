"""
Model references can be found in ARES, TMWF and query2label
"""
import copy
import torch
import numpy as np
...
if __name__ == '__main__':
    net = MW3F()#经过dfin:torch.Size([4, 1, 5000]) --> out:torch.Size([4, 21, 256])
    import numpy as np
    x1 = np.random.rand(4, 1,10000)
    x2 = np.random.rand(4, 1,10000)
    x1 = torch.tensor(x1, dtype=torch.float32)
    x2 = torch.tensor(x2, dtype=torch.float32)
    out = net((x1,x2))
    print(f"in:{x1.shape} --> out:{out.shape}")#